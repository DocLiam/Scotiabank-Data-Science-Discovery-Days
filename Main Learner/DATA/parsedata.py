...
for row in csv_filer:
    ...
    for header in row:
        ...
        if header.startswith("MEAN") and header.endswith("7DAY") and header.split("_PAST")[0].split("MEAN_")[1] in FLAG_headers:
            FLAG_count += 1.0
    ...
    if not test:
        if i%3 != 0 and ((float(row["FRAUD_FLAG"]) == 0 and count_FRAUD_train >= count_not_FRAUD_train) or float(row["FRAUD_FLAG"]) == 1):
            filew_train.write(",".join([str(value) for value in rest_values]) + ":" + ("0" if len(row["FRAUD_FLAG"]) == 0 else ("0" if float(row["FRAUD_FLAG"]) == 0 else "1.0")) + "\n")
            
            if float(row["FRAUD_FLAG"]) == 1:
                count_FRAUD_train += 1
            else:
                count_not_FRAUD_train += 1
        elif i%3 == 0 and ((float(row["FRAUD_FLAG"]) == 0 and count_FRAUD_validate >= count_not_FRAUD_validate) or float(row["FRAUD_FLAG"]) == 1):
            filew_validate.write(",".join([str(value) for value in rest_values]) + ":" + ("0" if len(row["FRAUD_FLAG"]) == 0 else ("0" if float(row["FRAUD_FLAG"]) == 0 else "1.0")) + "\n")
    ...
    if test:
        filew_test.write(",".join([str(value) for value in rest_values]) + ":" + ("0" if len(row["FRAUD_FLAG"]) == 0 else ("0" if float(row["FRAUD_FLAG"]) == 0 else "1.0")) + "\n")
